...
export const asyncPopulateFederalExpendituresData = async (db: Firestore, metric: Emetric_Metric): Promise<void> => {
  try {
    ...
    if (latestTimestamp) {
      ...
      logger.info(`Found existing Federal Expenditures data with latest timestamp: ${new Date(latestTimestamp * 1000).toISOString()}`);
      ...
      logger.info(`Fetching Federal Government Expenditures data from FRED since ${startDate}`);
      const expendituresData = await fredService.fetchMetricData(FEDERAL_EXPENDITURES_METRIC, startDate);
      
      if (expendituresData.length === 0) {
        logger.info("No new Federal Government Expenditures data available since last update");
        return;
      }
      
      // Transform new data to Emetric format
      const newEntries: Emetric_TimeSeriesEntry[] = expendituresData.map(item => ({
      ...
      logger.info(`Successfully added ${newEntries.length} new Federal Government Expenditures data points to existing ${existingEntries.length} points`);
    } else {
      // No existing data, fetch all data as before
      logger.info("No existing Federal Government Expenditures data found, fetching all available data");
