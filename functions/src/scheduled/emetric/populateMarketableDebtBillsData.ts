...
export const asyncPopulateMarketableDebtBillsData = async (db: Firestore, metric: Emetric_Metric): Promise<void> => {
  try {
    logger.info("Starting asyncPopulateMarketableDebtBillsData");
    ...
    if (latestTimestamp) {
      ...
      logger.info(`Found existing Marketable Debt Bills data with latest timestamp: ${new Date(latestTimestamp * 1000).toISOString()}`);
      ...
      logger.info(`Fetching Marketable Debt Bills data from Treasury API since ${startDate}`);
      const debtData = await treasuryService.fetchMarketableDebtBillsData(startDate);
      
      if (debtData.length === 0) {
        logger.info("No new Marketable Debt Bills data available since last update");
        return;
      }
      ...
      logger.info(`Successfully added ${newEntries.length} new Marketable Debt Bills data points to existing ${existingEntries.length} points`);
    } else {
      // No existing data, fetch all data
      logger.info("No existing Marketable Debt Bills data found, fetching all available data");
      
      // Fetch Marketable Debt Bills data
      logger.info("Fetching Marketable Debt Bills data from Treasury API");
      const debtData = await treasuryService.fetchMarketableDebtBillsData();
      
      if (debtData.length === 0) {
        logger.warn("No Marketable Debt Bills data returned from Treasury API");
        return;
      }
      ...
      logger.info(`Successfully stored ${timeSeriesEntries.length} Marketable Debt Bills data points`);
    }
  } catch (error) {
    logger.error("Error on asyncPopulateMarketableDebtBillsData:", error);
    throw error;
  }
};
