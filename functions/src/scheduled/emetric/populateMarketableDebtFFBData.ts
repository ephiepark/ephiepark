...
export const asyncPopulateMarketableDebtFFBData = async (db: Firestore, metric: Emetric_Metric): Promise<void> => {
  try {
    logger.info("Starting asyncPopulateMarketableDebtFFBData");
    ...
    if (latestTimestamp) {
      ...
      logger.info(`Found existing Marketable Debt FFB data with latest timestamp: ${new Date(latestTimestamp * 1000).toISOString()}`);
      ...
      logger.info(`Fetching Marketable Debt FFB data from Treasury API since ${startDate}`);
      const debtData = await treasuryService.fetchMarketableDebtFFBData(startDate);
      
      if (debtData.length === 0) {
        logger.info("No new Marketable Debt FFB data available since last update");
        return;
      }
      ...
      logger.info(`Successfully added ${newEntries.length} new Marketable Debt FFB data points to existing ${existingEntries.length} points`);
    } else {
      // No existing data, fetch all data
      logger.info("No existing Marketable Debt FFB data found, fetching all available data");
      
      // Fetch Marketable Debt FFB data
      logger.info("Fetching Marketable Debt FFB data from Treasury API");
      const debtData = await treasuryService.fetchMarketableDebtFFBData();
      
      if (debtData.length === 0) {
        logger.warn("No Marketable Debt FFB data returned from Treasury API");
        return;
      }
      ...
      logger.info(`Successfully stored ${timeSeriesEntries.length} Marketable Debt FFB data points`);
    }
  } catch (error) {
    logger.error("Error on asyncPopulateMarketableDebtFFBData:", error);
    throw error;
  }
};
