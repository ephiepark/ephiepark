...
export const asyncPopulateMarketableDebtFRNData = async (db: Firestore, metric: Emetric_Metric): Promise<void> => {
  try {
    logger.info("Starting asyncPopulateMarketableDebtFRNData");
    ...
    if (latestTimestamp) {
      ...
      logger.info(`Found existing Marketable Debt FRN data with latest timestamp: ${new Date(latestTimestamp * 1000).toISOString()}`);
      ...
      logger.info(`Fetching Marketable Debt FRN data from Treasury API since ${startDate}`);
      const debtData = await treasuryService.fetchMarketableDebtFRNData(startDate);
      
      if (debtData.length === 0) {
        logger.info("No new Marketable Debt FRN data available since last update");
        return;
      }
      ...
      logger.info(`Successfully added ${newEntries.length} new Marketable Debt FRN data points to existing ${existingEntries.length} points`);
    } else {
      // No existing data, fetch all data
      logger.info("No existing Marketable Debt FRN data found, fetching all available data");
      
      // Fetch Marketable Debt FRN data
      logger.info("Fetching Marketable Debt FRN data from Treasury API");
      const debtData = await treasuryService.fetchMarketableDebtFRNData();
      
      if (debtData.length === 0) {
        logger.warn("No Marketable Debt FRN data returned from Treasury API");
        return;
      }
      ...
      logger.info(`Successfully stored ${timeSeriesEntries.length} Marketable Debt FRN data points`);
    }
  } catch (error) {
    logger.error("Error on asyncPopulateMarketableDebtFRNData:", error);
    throw error;
  }
};
