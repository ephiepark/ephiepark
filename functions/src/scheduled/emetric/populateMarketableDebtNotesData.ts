...
export const asyncPopulateMarketableDebtNotesData = async (db: Firestore, metric: Emetric_Metric): Promise<void> => {
  try {
    logger.info("Starting asyncPopulateMarketableDebtNotesData");
    ...
    if (latestTimestamp) {
      ...
      logger.info(`Found existing Marketable Debt Notes data with latest timestamp: ${new Date(latestTimestamp * 1000).toISOString()}`);
      ...
      logger.info(`Fetching Marketable Debt Notes data from Treasury API since ${startDate}`);
      const debtData = await treasuryService.fetchMarketableDebtNotesData(startDate);
      
      if (debtData.length === 0) {
        logger.info("No new Marketable Debt Notes data available since last update");
        return;
      }
      ...
      logger.info(`Successfully added ${newEntries.length} new Marketable Debt Notes data points to existing ${existingEntries.length} points`);
    } else {
      // No existing data, fetch all data
      logger.info("No existing Marketable Debt Notes data found, fetching all available data");
      
      // Fetch Marketable Debt Notes data
      logger.info("Fetching Marketable Debt Notes data from Treasury API");
      const debtData = await treasuryService.fetchMarketableDebtNotesData();
      
      if (debtData.length === 0) {
        logger.warn("No Marketable Debt Notes data returned from Treasury API");
        return;
      }
      ...
      logger.info(`Successfully stored ${timeSeriesEntries.length} Marketable Debt Notes data points`);
    }
  } catch (error) {
    logger.error("Error on asyncPopulateMarketableDebtNotesData:", error);
    throw error;
  }
};
