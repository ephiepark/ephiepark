...
export const asyncPopulateMarketableDebtTIPSData = async (db: Firestore, metric: Emetric_Metric): Promise<void> => {
  try {
    logger.info("Starting asyncPopulateMarketableDebtTIPSData");
    ...
    if (latestTimestamp) {
      ...
      logger.info(`Found existing Marketable Debt TIPS data with latest timestamp: ${new Date(latestTimestamp * 1000).toISOString()}`);
      ...
      logger.info(`Fetching Marketable Debt TIPS data from Treasury API since ${startDate}`);
      const debtData = await treasuryService.fetchMarketableDebtTIPSData(startDate);
      
      if (debtData.length === 0) {
        logger.info("No new Marketable Debt TIPS data available since last update");
        return;
      }
      ...
      logger.info(`Successfully added ${newEntries.length} new Marketable Debt TIPS data points to existing ${existingEntries.length} points`);
    } else {
      // No existing data, fetch all data
      logger.info("No existing Marketable Debt TIPS data found, fetching all available data");
      
      // Fetch Marketable Debt TIPS data
      logger.info("Fetching Marketable Debt TIPS data from Treasury API");
      const debtData = await treasuryService.fetchMarketableDebtTIPSData();
      
      if (debtData.length === 0) {
        logger.warn("No Marketable Debt TIPS data returned from Treasury API");
        return;
      }
      ...
      logger.info(`Successfully stored ${timeSeriesEntries.length} Marketable Debt TIPS data points`);
    }
  } catch (error) {
    logger.error("Error on asyncPopulateMarketableDebtTIPSData:", error);
    throw error;
  }
};
