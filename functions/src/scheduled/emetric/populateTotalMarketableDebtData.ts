...
export const asyncPopulateTotalMarketableDebtData = async (db: Firestore, metric: Emetric_Metric): Promise<void> => {
  try {
    logger.info("Starting asyncPopulateTotalMarketableDebtData");
    ...
    if (latestTimestamp) {
      ...
      logger.info(`Found existing Total Marketable Debt data with latest timestamp: ${new Date(latestTimestamp * 1000).toISOString()}`);
      ...
      logger.info(`Fetching Total Marketable Debt data from Treasury API since ${startDate}`);
      const debtData = await treasuryService.fetchTotalMarketableDebtData(startDate);
      
      if (debtData.length === 0) {
        logger.info("No new Total Marketable Debt data available since last update");
        return;
      }
      ...
      logger.info(`Successfully added ${newEntries.length} new Total Marketable Debt data points to existing ${existingEntries.length} points`);
    } else {
      // No existing data, fetch all data
      logger.info("No existing Total Marketable Debt data found, fetching all available data");
      
      // Fetch Total Marketable Debt data
      logger.info("Fetching Total Marketable Debt data from Treasury API");
      const debtData = await treasuryService.fetchTotalMarketableDebtData();
      
      if (debtData.length === 0) {
        logger.warn("No Total Marketable Debt data returned from Treasury API");
        return;
      }
      ...
      logger.info(`Successfully stored ${timeSeriesEntries.length} Total Marketable Debt data points`);
    }
  } catch (error) {
    logger.error("Error on asyncPopulateTotalMarketableDebtData:", error);
    throw error;
  }
};
