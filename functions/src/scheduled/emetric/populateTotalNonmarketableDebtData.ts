...
export const asyncPopulateTotalNonmarketableDebtData = async (db: Firestore, metric: Emetric_Metric): Promise<void> => {
  try {
    logger.info("Starting asyncPopulateTotalNonmarketableDebtData");
    ...
    if (latestTimestamp) {
      ...
      logger.info(`Found existing Total Nonmarketable Debt data with latest timestamp: ${new Date(latestTimestamp * 1000).toISOString()}`);
      ...
      logger.info(`Fetching Total Nonmarketable Debt data from Treasury API since ${startDate}`);
      const debtData = await treasuryService.fetchTotalNonmarketableDebtData(startDate);
      
      if (debtData.length === 0) {
        logger.info("No new Total Nonmarketable Debt data available since last update");
        return;
      }
      ...
      logger.info(`Successfully added ${newEntries.length} new Total Nonmarketable Debt data points to existing ${existingEntries.length} points`);
    } else {
      // No existing data, fetch all data
      logger.info("No existing Total Nonmarketable Debt data found, fetching all available data");
      
      // Fetch Total Nonmarketable Debt data
      logger.info("Fetching Total Nonmarketable Debt data from Treasury API");
      const debtData = await treasuryService.fetchTotalNonmarketableDebtData();
      
      if (debtData.length === 0) {
        logger.warn("No Total Nonmarketable Debt data returned from Treasury API");
        return;
      }
      ...
      logger.info(`Successfully stored ${timeSeriesEntries.length} Total Nonmarketable Debt data points`);
    }
  } catch (error) {
    logger.error("Error on asyncPopulateTotalNonmarketableDebtData:", error);
    throw error;
  }
};
